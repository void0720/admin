    ...
    $.fn.extend({
        select: function (options) {
            ...
            var defaults = {
                ...
                minWidth: "100px",              //最小宽度值,默认不设置（特殊注明：单位只能是px）
                ...
                rowColor: "#ccc",          //行原本的颜色,此处根据页面背景颜色设置,不设置会默认白色
                rowHoverColor: "#222d32", //移动选择时，每一行的hover底色
            ...
            $this.hide();
            $this.after('\
                    <div class="select_container_nw" sign='+ randomSign + ' tabindex="0" val="" text="">\
            ...
            if (opts.rowColor != "" && opts.rowColor != "#fff") {
                $This.find(".select_list,.select_input").css("background-color", opts.rowColor);
            }
            if (opts.rowHoverColor != "") {
                $This.find("li").hover(function () {
                    $(this).css({ "background-color": opts.rowHoverColor });
                }, function () {
                    $(this).css({ "background-color": opts.opacity ? "transparent" : opts.rowColor });
                });
            }
            ...
            $This.find(".select_arrow").css({ "display": "block", "height": $This.find(".select_arrow").find("i").height()+8 + "px", "padding": ($This.find(".select_main").height() - $This.find(".select_arrow").find("i").height()) / 2 + "px 0px", "line-height": $This.find(".select_arrow").find("i").height() + 2 + "px" });
            ...
            $This.find(".select_list_body .mCSB_dragger_bar").css({ "background-color": "#888" });

            //传进来的参数操作，需要写在滚动插件调用之后
            if (opts.themeColor != "" && opts.themeColor != "#888") {
